    ...
    if tf:
        for r in await check_traffy(user_id, tf):
            if r.get("status") == "completed":
                ...
                await _create_hold(user_id, "traffy", aid, rw)
                done_count += 1; done_sum += rw
            ...
            for h in holds:
                ...
                try:
                    if service == "piarflow":
                        ...
                        r = await check_traffy(uid, [aid])
                        still = any(x.get("status") == "completed" for x in r)
...
async def cb_menu(cq: types.CallbackQuery):
    missing = await check_required(cq.from_user.id)
    if missing:
        await cq.answer("❌ Сначала подпишись", show_alert=True)
        return
    u = await get_user(cq.from_user.id)
    bal = u["balance"] if u else 0
    held = u["held"] if u else 0
...
async def cb_balance(cq: types.CallbackQuery):
    ...
    try:
        await cq.message.edit_text(
            f"💰 Доступно: <b>{bal:.2f} BC</b>\n⏳ В холде: <b>{held:.2f} BC</b>\n📤 Минимум: <b>{MIN_WITHDRAW:.0f} BC</b>",
            reply_markup=main_menu())
    except Exception:
        pass
...
async def cb_earn(cq: types.CallbackQuery):
    ...
    if not all_links:
        try:
            await cq.message.edit_text("😕 Заданий нет, позже.", reply_markup=main_menu())
        except Exception:
            pass
        return
    try:
        await cq.message.edit_text(
            ...
            f"⏳ Награда через <b>{HOLD_HOURS}ч</b> (если не отпишешься).",
...
async def cb_check(cq: types.CallbackQuery):
    ...
    if cnt > 0:
        ...
        try:
            ...
            try: await cq.message.answer(text, reply_markup=main_menu())
...
async def cb_refs(cq: types.CallbackQuery):
    ...
    async with pool.acquire() as db:
        ...
        refs_count = row["c"]
        top_refs = await db.fetch("SELECT user_id, referred_earned FROM users WHERE referrer_id=$1 ORDER BY referred_earned DESC LIMIT 10", cq.from_user.id)
    await cq.answer()
...
async def cb_promo(cq: types.CallbackQuery):
    ...
    await cq.answer()
    try:
        await cq.message.edit_text("🎁 Отправь промокод")
    except Exception:
        pass
...
async def cb_withdraw(cq: types.CallbackQuery):
    ...
    bal = u["balance"] if u else 0
    if bal < MIN_WITHDRAW:
        await cq.answer(f"❌ Минимум {MIN_WITHDRAW:.0f} BC. У тебя {bal:.2f}", show_alert=True)
        return
    _states[cq.from_user.id] = "await_withdraw"
    await cq.answer()
    try:
        await cq.message.edit_text(
            f"💸 <b>Вывод</b>\n\n💰 Доступно: <b>{bal:.2f} BC</b>\n\nОтправь сумму:\n<code>1500</code>")
...
async def text_handler(msg: types.Message):
    ...
    if state == "await_promo":
        ...
        async with pool.acquire() as db:
            ...
            if used:
                await msg.answer("❌ Ты уже активировал")
                _states.pop(uid, None)
                return
    ...
    if state == "await_withdraw":
        ...
        if amount < MIN_WITHDRAW:
            await msg.answer(f"❌ Минимум {MIN_WITHDRAW:.0f} BC")
            return
        u = await get_user(uid)
        bal = u["balance"] if u else 0
        if amount > bal:
            await msg.answer(f"❌ У тебя только {bal:.2f} BC")
            return
        _states.pop(uid, None)
        pool = await get_pool()
        async with pool.acquire() as db:
            ...
            wid = row["id"]
        await msg.answer(f"⏳ Вывод {amount:.2f} BC обрабатывается…")
        ...
        if resp.get("status") == "ok":
            ...
            await msg.answer(f"❌ Ошибка вывода: {resp.get('error','unknown')}\nСредства возвращены.")
        return
...
async def forward_req(msg: types.Message):
    ...
    if _states.get(uid) != "adm_forward": return
