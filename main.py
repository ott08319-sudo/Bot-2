...
def sponsors_kb(links):
    ...
    # по 2 в ряд + отдельная строка для кнопки проверки
    rows = [2] * (len(links) // 2)
    if len(links) % 2:
        rows.append(1)
    rows.append(1)
...
async def save_required_and_finish(uid, buf):
    ...
    async with pool.acquire() as db:
        try:
            await db.execute("""INSERT INTO required_channels 
                (chat_id, title, url, type, active, created_at)
                VALUES ($1,$2,$3,$4,TRUE,$5)
                ON CONFLICT (chat_id) DO UPDATE 
                SET title=EXCLUDED.title, url=EXCLUDED.url, type=EXCLUDED.type, active=TRUE""",
                (buf["chat_id"], buf["title"], buf["url"], buf["type"], time.time()))
        except Exception as e:
            await bot.send_message(uid, f"❌ Ошибка: <code>{e}</code>", reply_markup=admin_menu())
            return
    ...
    await bot.send_message(
        uid,
        f"✅ <b>Добавлено!</b>\n\n"
        f"{type_names.get(buf['type'], buf['type'])} <b>{buf['title']}</b>\n"
        f"ID: <code>{buf['chat_id']}</code>\n"
        f"URL: {buf['url']}",
        ...
        try:
            ...
            if method == "POST":
                async with s.post(url, headers=headers or {}, json=json_data or {}, timeout=timeout) as r:
                    ...
                    body = await r.text()
                    if r.status == 200:
                        return (name, True, ms, f"OK")
                    return (name, False, ms, f"HTTP {r.status}")
            else:
